...
const DrawingPage = GObject.registerClass({
    ...
    constructor(extensionPreferences, window) {
        ...
        const importButton = Gtk.Button.new_from_icon_name("document-open-symbolic");
        ...
        const backgroundColorRow = Adw.ActionRow.new();
        backgroundColorRow.set_title(this.schema.get_key("background-color").get_summary());

        const backgroundColorButton = new ColorStringButton({ use_alpha:    true, show_editor:  true,
        ...
        const gridLineRow = Adw.ActionRow.new();
        gridLineRow.set_title(_("Grid overlay line"));

        const gridLineAutoButton = new Gtk.CheckButton({ label:        _("Auto"),
                                                         name:         this.schema.get_key("grid-line-auto").get_summary(),
                                                         tooltip_text: this.schema.get_key("grid-line-auto").get_description() });
        const gridLineWidthButton = new PixelSpinButton({ width_chars:  5, digits:       1, step:         0.1,
        ...
        const gridLineSpacingButton = new PixelSpinButton({ width_chars:  5, digits:       1, step:         1,
        ...
        const gridColorRow = Adw.ActionRow.new();
        gridColorRow.set_title(this.schema.get_key("grid-color").get_summary());

        const gridColorButton = new ColorStringButton({ use_alpha:    true, show_editor:  true,
        ...
        /* Tools Group */
        const adw_groupTools = Adw.PreferencesGroup.new();
        adw_groupTools.set_title(_("Tools"));


        const dashArrayRow = Adw.ActionRow.new();
        dashArrayRow.set_title(_("Dash array"));

        const dashArrayAutoButton = new Gtk.CheckButton({ label:        _("Auto"),
                                                          name:         this.schema.get_key("dash-array-auto").get_summary(),
                                                          tooltip_text: this.schema.get_key("dash-array-auto").get_description() });
        const dashArrayOnButton = new PixelSpinButton({ width_chars:  5, digits:       1, step:         0.1,
        ...
        const dashArrayOffButton = new PixelSpinButton({ width_chars:  5, digits:       1, step:         0.1,
        ...
        const dashOffsetRow = Adw.ActionRow.new();
        dashOffsetRow.set_title(this.schema.get_key("dash-offset").get_summary());

        const dashOffsetButton = new PixelSpinButton({ width_chars:  5, digits:       1, step:         0.1,
        ...
        const imageLocationRow = Adw.ActionRow.new();
        imageLocationRow.set_title(this.schema.get_key("image-location").get_summary());

        const imageLocationButton = new FileChooserButton({ action:       Gtk.FileChooserAction.SELECT_FOLDER,
        ...
        const resetButton = new Gtk.Button({ label: _("Reset settings"), halign: Gtk.Align.CENTER });
    ...
    _updatePalettes() {
        ...
        this.palettes.forEach((palette, index) => {
            const tmpAction = Adw.ActionRow.new();
            tmpAction.set_title(palette[0]);
            this.nPalettes.push(tmpAction);

            const EditButton = Gtk.Button.new_from_icon_name("edit-symbolic");
            ...
            const RemoveButton = Gtk.Button.new_from_icon_name("edit-delete-symbolic");
    ...
    _onPaletteColorChanged(paletteIndex, colorIndex, colorButton) {
        ...
        if (colorButton.tooltip_text) { this.palettes[paletteIndex][1][colorIndex] += `:${colorButton.tooltip_text}`; }
        this._savePalettes();
    }
    ...
    _importPalette() {
        ...
        dialog.connect("response", (dialog, response) => {
            if (response == Gtk.ResponseType.ACCEPT) {
                ...
                const palettes = GimpPaletteParser.parseFile(file);
    ...
    _editPalette(palette, index) {
        ...
        const preferencesPage = Adw.PreferencesPage.new();
        const editor_palettes_group = Adw.PreferencesGroup.new();
        editor_palettes_group.set_title(_("Customize Palette"));

        const PalettesNameActionRow = Adw.ActionRow.new();
        PalettesNameActionRow.set_title(_("Name"));
        const EntryName = Gtk.EditableLabel.new(palette[0]);
        ...
        palette[1].forEach((color, colorindex)=>{
            const pActionRow = Adw.ActionRow.new();
            const [cname, title] = color.split(":");

            const colortitle = Gtk.EditableLabel.new(title || cname);
            ...
            const colorEntry = new ColorStringButton({ use_alpha:    true, show_editor:  true,
                                                       name:         cname,
...
const PixelSpinButton = new GObject.Class({
    ...
    set range(range) {
        const [type, variant] = range.deep_unpack();
        if (type == "range") {
            const [min, max] = variant.deep_unpack();
...
const FileChooserButton = new GObject.Class({
    ...
    vfunc_clicked: function() {
        ...
        if (this.location) { dialog.set_file(Gio.File.new_for_commandline_arg(this.location)); }

        dialog.connect("response", (dialog, response) => {
            if (response == Gtk.ResponseType.ACCEPT) { this.location = dialog.get_file().get_path(); }
            dialog.destroy();
        });
