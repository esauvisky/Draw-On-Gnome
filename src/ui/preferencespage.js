...
const PreferencesPage = GObject.registerClass({
    ...
    constructor(extensionPreferences) {
        ...
        const settings = extensionPreferences.getSettings();
        const schema = settings.settings_schema;

        const grp_Global = Adw.PreferencesGroup.new();
        grp_Global.set_title("Global");

        Shortcuts.GLOBAL_KEYBINDINGS.forEach((settingKeys) => {
            const globalKeybindingsRow = new Adw.ActionRow();
            const globalKeybindingsWidget = new ShortCutWidget(settings, settingKeys);
            const name = settings.settings_schema.get_key(settingKeys).get_summary();
        ...
        Shortcuts.GLOBAL_KEYBINDINGS_SWITCHES.forEach((settingKeys) => {
            ...
            const ActionRow_switch = Gtk.Switch.new();
            const persistentOverTogglesKey = schema.get_key(settingKeys);

            ActionRow.set_title(persistentOverTogglesKey.get_summary());
            const description = persistentOverTogglesKey.get_description();

            if (description) { ActionRow.set_subtitle(persistentOverTogglesKey.get_description()); }
        ...
        const grp_Internal = Adw.PreferencesGroup.new();
        grp_Internal.set_title("Internal");

        const internalShortcutSettings = extensionPreferences.getSettings(extensionPreferences.metadata["settings-schema"] + ".internal-shortcuts");

        // TODO: Improve Shortcut Widget
        Shortcuts.INTERNAL_KEYBINDINGS.forEach((settingKeys) => {
            const globalKeybindingsRow = new Adw.ActionRow();
            const globalKeybindingsWidget = new ShortCutWidget(internalShortcutSettings, settingKeys);
            const name = internalShortcutSettings.settings_schema.get_key(settingKeys).get_summary();
...
const ShortCutWidget = GObject.registerClass({
    ...
    constructor(settings, settingsKeys) {
        ...
        const key = settings.get_strv(settingsKeys)[0];
        ...
        if (key == null) { this.set_label(_("Click here to set new shortcut...")); } else { this.set_child(this.shortcutlabel); }
    ...
    on_settings_changed() {
        const key = this._settings.get_strv(this._settingsKeys)[0];
    ...
    key_released(widget, keyval, keycode, state) {
        if (this.allow_changes == false) { return Gdk.EVENT_STOP; } else { this.allow_changes = false; }

        const value = this.shortcutlabel.get_accelerator();
        this._settings.set_strv(this._settingsKeys, [value]);
    }
    key_pressed(widget, keyval, keycode, state) {
        if (this.allow_changes == false) { return Gdk.EVENT_STOP; }
